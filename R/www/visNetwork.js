...
HTMLWidgets.widget({
  ...
  renderValue: function(el, x, instance) {
    ...
    maindiv.setAttribute('style', 'height:100%;background-color: inherit;margin-top:-10px');
    ...
    var tempX = 0
    var tempY = 0
    // if element is in a modal, adjust the XY
    // console.log("Element")
    // console.log(el)
    // console.log("el:", $(el).attr("id"))
    var modalDialog = $(el).parents(".modal-dialog")
    ...
    function getMouseXY(e) {
      tempX = e.clientX
      tempY = e.clientY
      // apply offset if in modal
      if (modalDialog.length) {
        tempX -= modalDialog.offset().left
        tempY -= modalDialog.offset().top
      }
   ...
   //this.body.emitter.emit("showPopup",{id:this.popupObj.id,x:t.x+3,y:t.y-5}))
    ...
    vispopup.setAttribute('class', 'visnetwork-tooltip visnetwork-tooltip-top')
    ...
    // make tooltip stay with mouse
    function moveTooltip(e) {
        vispopup.style.top = tempY - vispopup.offsetHeight + "px";
        vispopup.style.left = tempX - vispopup.offsetWidth/2 + "px";
    }
    ...
    instance.network.on("hoverNode", function(params){
      if(el_id.hoverNearest && x.nodes){
        instance.network.canvas.body.container.style.cursor = "pointer";
    ...
    instance.network.on("blurNode", function(params){
      if(el_id.hoverNearest && x.nodes){
        instance.network.canvas.body.container.style.cursor = "default";
